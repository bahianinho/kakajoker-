	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Até mais @${num.split('@')[0]}, um abraço do bahianinho👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '📍Este comando só pode ser usado pelo bot proprietário!⚠️',
			...
			switch(command) {
				...
				case 'sticker':
					...
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Cade o codigo do idioma', text, {quoted: mek})
				...
				case 'listadmins':
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ CADE O STICKER?❌')
					reply(mess.wait)
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Pronto, Ativado, lindo!✔️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Desativou já, lindo!✔️')
					} else {
						reply('1 untuk mengaktifkan, 0 untuk menonaktifkan')
					}
				case 'clone':
